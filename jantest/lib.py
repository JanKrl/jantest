...
def try_me():
    ...
    lettersOnScreen = (int(displLength[0] / letterSize[0]),
    ...
    colorList += [(R - 50 if R else 0, B - 50 if B else 0, G - 50 if G else 0),
    ...
    # char generator
    letters = [[0 for _ in range(lettersOnScreen[1] + 1)]
            for _ in range(lettersOnScreen[0])]
    if ZERO_ONE:
    ...
    for y in range(lettersOnScreen[1] + 1):
        for x in range(lettersOnScreen[0]):
            letters[x][y] = [
                fontObj.render(char, False, colorList[col])
    ...
    if len(str) > 0:
        ...
        for x in range((lettersOnScreen[0] / 2) - (len(str) / 2),
            ...
            letters[x][lettersOnScreen[1] / 2] = [
                fontObj.render(
                    str[x - ((lettersOnScreen[0] / 2) - (len(str) / 2))], False,
                    (255, 255, 255)) for col in range(maxCol)
            ]
    ...
    while ticksLeft > 0 and (notDone) and (wordMode):
        ...
        for x in range(lettersOnScreen[0]):
            ...
            while (counter > 0) and (col < maxCol):
                if (counter < lettersOnScreen[1] + 2) and (col < primeColors
                                                        or col >
                                                        (maxCol - endColors)):
                    surface.blit(letters[x][counter - 1][col],
                                (x * letterSize[0],
    ...
    for y in range(int(lettersOnScreen[1] / 2 + 1), int(lettersOnScreen[1] + 1)):
        for x in range(int((lettersOnScreen[0] / 2) - (len(str) / 2)),
